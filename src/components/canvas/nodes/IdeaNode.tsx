...
function IdeaNodeComponent({ id, data, selected }: NodeProps<IdeaNodeData>) {
  ...
  return (
    ...
    >
      ...
      <NodeResizer minWidth={200} minHeight={140} isVisible={selected} color="#3b82f6" lineStyle={{ borderWidth: 1 }} handleStyle={{ width: 6, height: 6, borderRadius: 2 }} />
      <Handle type="target" position={Position.Left} className="!-left-[5px] !h-3 !w-3 !rounded-full !border-2 !border-blue-300 !bg-white" />

      <div className="shrink-0 pl-4 pr-3 py-2.5">
        <div className="text-[11px] font-semibold uppercase tracking-wider text-blue-500">💡 Idea</div>
        <div className="truncate text-[11px] text-zinc-400 mt-0.5">{content ? content.slice(0, 45) : "Capture an idea from the meeting..."}</div>
      </div>
      ...
      <Handle type="source" position={Position.Right} className="!-right-[5px] !h-3 !w-3 !rounded-full !border-2 !border-blue-300 !bg-white" />
    </div>
  );
}
